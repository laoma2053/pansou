...
const SEARCH_CONFIG = {
  ...
  // 数据来源配置
  dataSource: "tg", // 数据来源：all(全部)、tg(仅TG)、plugin(仅插件)
...
async function executeSearch(keyword) {
  ...
  try {
    const searchParams = {
      kw: keyword,
      src: SEARCH_CONFIG.dataSource, // 🎯 使用配置的数据来源
    ...
    try {
      ...
      throw new Error(
        `JSON解析失败: ${parseError.message}. 响应长度: ${responseText.length}`
      );
    }
...
function generateTabs(mergedData) {
  ...
  if (currentResults) {
    ...
    if (currentResults.total !== undefined) {
      ...
      Array.isArray(currentResults.results)
    ) {
...
function checkTabsScrollArrow() {
  ...
  if (tabsContainer.scrollLeft >= maxScrollLeft - 1) {
    // -1 容错
    rightArrow.style.display = "none";
    console.log("隐藏右箭头");
  } else {
    ...
    resultDiv.innerHTML = `
            ...
            <div class="result-meta">
                <span class="result-date">${
                  result.datetime
                    ? `资源分享时间：${formatDate(result.datetime)}`
                    : ""
                }</span>
            </div>
            ${
              links.length > 0
                ? `<div class="result-netdisk-links">${linksHtml}</div>`
                : ""
            }
        `;
...
function createLinkItem(link) {
  ...
  const note = cleanTitle
    ? `<div class="link-title"><a href="${
        link.url
      }" target="_blank">${escapeHtml(cleanTitle)}</a></div>`
    : "";
  const date = link.datetime
    ? `<div class="link-date">${formatDate(link.datetime)}</div>`
    : "";
  ...
  return `
        <div class="link-item">
            ...
            <div class="link-url"><a href="${
              link.url
            }" target="_blank">${escapeHtml(link.url)}</a></div>
...
function displaySpecificNetdiskType(netdiskType) {
  ...
  // 只显示选中的网盘类型
  const targetGroup = mergedResults.querySelector(
    `[data-netdisk-type="${netdiskType}"]`
  );
  if (targetGroup) {
...
function showLoading() {
  ...
  if (mergedResults) {
    ...
    const randomSubMessage =
      loadingSubMessages[Math.floor(Math.random() * loadingSubMessages.length)];
